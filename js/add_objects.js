...
function additem() {
    ...
    var pic = document.getElementById("item_pic").files[0];
            ...
            socket.on('getsize_Unit', (data) => {
                socket.on('getweight_Unit', (dat) => {
                    ...
                    if (!(pic == null || pic == "" || pic == " ")) {
                        console.log(pic);
                        if (pic.size > 9990) {
                            socket.emit('item_data', [name, instructions, size, color, true, quantity, Math.ceil(pic.size / 9900), weight, price]);
                            socket.on("successi", (data) => {
                                //TODO
                                document.getElementById("item_pic_f").reset(); location.reload();
                            });
                        } else {
                            socket.emit('item_data', [name, instructions, size, color, pic, quantity, pic.size, weight, price]);
                            socket.on("successi", (data) => {
                                document.getElementById("item_pic_f").reset(); location.reload();
                            });
                        }
                    } else {
                        socket.emit('item_data', [name, instructions, size, color, false, quantity, 0, weight, price]);
                        socket.on("successi", (data) => {
                            document.getElementById("item_pic_f").reset(); location.reload();
                        });
                    }
